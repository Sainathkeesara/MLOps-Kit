...
## Monitoring & Drift
- **Concept drift** — A change in the relationship between features and the target variable itself, so a model's assumptions become wrong even when input distributions look familiar.
- **Baseline snapshot** — A frozen reference dataset saved at training time (the feature values the model saw during validation), used as the comparison point for every subsequent drift check.
- **Kolmogorov-Smirnov (KS) test** — A statistical test comparing two sample distributions that is commonly used to detect per-feature drift, with a p-value cutoff (e.g. p < 0.05) flagging meaningful shift.
- **Population Stability Index (PSI)** — A metric quantifying how much a feature's distribution has changed between a reference and current dataset; PSI > 0.2 is a common threshold for meaningful drift.
- **Scheduled batch check** — Drift detection run on a rolling window (e.g. daily on the last 7 days of predictions) that outputs a pass/fail result, simpler than real-time streaming for batch prediction workloads.
- **Retraining trigger** — Wiring a drift check result into a pipeline orchestrator so exceeding the threshold kicks off a retraining pipeline run.
- **Drift report archive** — Persisting every drift report with a timestamp and the dataset hash that was checked, providing evidence of when drift started and progressed.
