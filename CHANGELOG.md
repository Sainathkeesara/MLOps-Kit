...
- wnb-053: docs — Removed dead link to `reports/metrics.json` from W&B Artifacts deep-dive (L3)
- con-114: docs — Removed dead link to `/app/serve.py` from multi-stage Dockerfile for MLOps notes (L2)
...
- con-108 (rework 2): docs — Experiment Tracking in real MLOps projects: patterns and pitfalls (L3) — added missing mlflow.org automation best-practices source to research.md, re-verified all 5 source URLs, updated front-matter date
...
- wnb-047: notes — Install W&B and log my first run (L1)
- wnb-048: snippet(python) — Log my first W&B metric with the Python SDK (L1)
...
- dvc-003: script(bash) — End-to-end DVC CLI walkthrough: init repo, track dataset with `dvc add`, set local remote, push, and verify cache contents (L1)
...
- kub-030 (rework): template — Removed unbacked flake8==7.0.0 hard version pin from CI/CD workflow (L3)
...
- con-115: script(python) — Combining Model Serving with Containerization for ML deployment (L3, rework — removed unbacked version pins)
