...
# Metaflow CI/CD with GitHub Actions: a first-run setup

> Notes from wiring Metaflow into GitHub Actions for the first time — what I set up, where I tripped, and what I'd change next.

## What I wanted

Every push to run a Metaflow flow automatically and fail the commit if the flow breaks. I wanted GitHub Actions to install dependencies, run the flow, and report results back to the PR — the same loop I use for unit tests, but for an ML pipeline.

## Step 1 — basic workflow

I started with a minimal workflow file:
...
Pushed it. The runner installed everything and the flow started, then died on `Flow is using @conda decorator but conda is not installed`.

## Step 2 — adding conda

Several of my steps use `@conda` to pin library versions. GitHub Actions runners don't have conda by default. I added the miniconda setup action:
...
Metaflow defaults to a local metadata service. In CI, each run starts fresh so the local metadata works fine — no remote metadata service needed. But my flow also calls `Flow.get_latest_successful_run()` to compare against a previous run. That fails on the first CI run because there is no previous run.
...
By default Metaflow stores artifacts in `$METAFLOW_HOME`. In GitHub Actions, the runner gets a fresh filesystem each time so artifacts from previous CI runs don't persist. That's fine for now — I'm just validating the flow runs without errors. If I wanted cross-run artifact persistence I'd need S3.
